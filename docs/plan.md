...
> **Status: SHIPPED in v0.2.0.0** — see CHANGELOG.md and `docs/product-spec.md §Frontend` for the as-built description. This section is preserved as the original direction reference. Implementation added a few capabilities not in this brief: a clickable DVD cover lightbox (handwritten VHS labels are unreadable at 60px), `···` discontinuity markers that condense empty-year runs on the top scrubber (so dragging snaps directly between content years instead of crawling through dead zones), and a drag-cache + O(1) active-element refactor for the scrubber hot path. Also: spine chevrons hidden on mobile (the top-bar pair is the sole stepper to avoid duplicate tap targets).

Generated by `/design-shotgun` on 2026-04-08 against the shipped v0.1.2.0 landing page. Two rounds, 8 initial variants + 3 synthesis remixes. This was a **direction reference for a future iteration**, not a v1 requirement.
...
**Scope note (historical):** When this section was written, the layout was *not* in v1 scope (v0.1.2.0 had just shipped). It was prioritized and built immediately afterward as v0.2.0.0.
